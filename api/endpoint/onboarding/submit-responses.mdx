...
<ParamField body="responses" type="array" required>
  ...
  May be empty (`[]`) **only when `mark_complete: true`** — useful for finalizing onboarding after answers were saved incrementally over earlier calls. An empty array with `mark_complete: false` is rejected with a `400 VALIDATION_ERROR`. When the array is empty, the `affiliate.onboarding_response_submitted` webhook is skipped (no answers were upserted).
