...
<Info>
  Every create requires `referral_id`, a positive `sale_amount`, and
  `commission_amount` (may be `0`). There is no standalone non-sale cash bonus
  endpoint. Currencies default to `USD`.
</Info>
