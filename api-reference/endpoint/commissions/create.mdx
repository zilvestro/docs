...
<ParamField body="affiliate_id" type="string" required>
  The unique identifier of the affiliate who will receive this commission. The affiliate must belong to your team.
</ParamField>
...
<ParamField body="amount_currency" type="string" default="USD">
...
<ParamField body="sales_status" type="string" default="open">
  The sales/transaction status. Valid values: `open` (sale is open/pending), `complete` (sale completed successfully), `trialing` (customer is in trial period), `failed` (sale failed), `refunded` (sale fully refunded), `partial_refunded` (sale partially refunded). Defaults to `open`.
</ParamField>
...
<ParamField body="invoice_id" type="string">
  Optional invoice ID from your billing system. Used to link the commission to an invoice.
</ParamField>
